...
for(id in 2:1){
  ...
  number_of_splits <- 7 #MODÃÂ°FY -> number of train,test pairs
  ...
  ms <- measures(testingData$sales, testingData$mean, testingData$sales, benchmark = "naive")
  ForecastAndID <- cbind(product_id = ids[id],
                         categorization = categorizationName,
                         forecastingGroup = paste("mean", collapse=","),
                         t(as.data.frame(ms)))
