...
ts_categorization <- data.frame(id = row.names(t(new_df)),
                                cv2 = ts_cate_obj$cv2,
                                p = ts_cate_obj$p) %>%
...
#graphs for some products
#prod_y <- filter(ydata, product_id == 939173536983210)[, -1] ## product y data

#ggplot(data=prod_y, aes(x=dates, y=sales)) +
