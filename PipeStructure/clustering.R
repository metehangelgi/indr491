...
inputCategorization <- c("dataCategorization/new", numofSample,"SBC.csv")
...
patterns <- c("product_id","size_.*","gender_.*")
...
categorized_dataCategories <- list("Smooth","Lumpy","Intermittent","Erratic")

for (a in c(1:length(categorized_dataCategories))){
  SBCtype<-categorized_dataCategories[[a]]
  filtered <- categorized_data %>%
    filter(demand_cate==SBCtype)
...
for (i in c(1:length(categorized_dataIDs))){
  ...
  SBCtype<-categorized_dataCategories[[i]]
